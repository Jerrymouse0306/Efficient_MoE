...
def get_sparse_csr(in_dim, out_dim, mask, device, fp16):
...
class RandomMaskingLinear(torch.nn.Module):
    def __init__(self, base_Linear: nn.Linear, in_dim: int, out_dim: int, mask: torch.tensor, fp16: bool):
        super().__init__()
        ...
        sparse_csr = get_sparse_csr(in_dim, out_dim, mask, base_Linear.weight.device, fp16)
...
class RandomMasking:
    def __init__(self, model, masking_prob, fo_optim_layers, fp16):
        self.model = model
        self.masking_prob = masking_prob
        assert 0.0 <= masking_prob <= 1.0
        self.mask_dict = {}
        self.fo_optim_layers = fo_optim_layers

        for key, _ in self.model.named_parameters():
            if len(self.fo_optim_layers) > 0:
                for layer in self.fo_optim_layers:
                    if layer in key:
                        if 'weight' in key and 'embed' not in key and 'lm_head' not in key and 'norm' not in key and 'Norm' not in key:
                            module_name = key.split(".")[-2]
                            module_key = '.'.join(key.split(".")[:-1])
                            parent_module, sub_key, module = find_module(self.model, module_key)
                            logger.info(key)
                            ...
                            logger.info(f"true masking prob: {torch.sum(mask) / torch.prod(torch.tensor(mask.shape))}")
