...
DEV=${DEV:-500}
EVAL=${EVAL:-1000}
STEPS=${STEPS:-1000} #20000
EVAL_STEPS=${EVAL_STEPS:-100}

MODE=${MODE:-ft-with-weight_wise-hybrid-opt}
EXTRA_ARGS=""
if [ "$MODE" == "prefix" ]; then
...
TAG=mezo-$MODE-last-20layer-adam-step-1-no-dropout-$STEPS-$BS-$FO_LR-$ZO_LR-$EPS-$SEED
...
CUDA_VISIBLE_DEVICES=0 python run.py \
    ...
    --max_steps $STEPS \
    --trainer zo --load_float16 --fo_optimizer "adam" \
    --hybrid_optimizer True --sgd_optim_step 1 --weight_wise_hybrid True --masking_prob 0.5 --alternate_training False --if_sgd_dropout False \
