...
if is_torch_xla_available(False):
...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        # )
        delay_optimizer_creation = is_sagemaker_dp_enabled() or self.is_fsdp_xla_enabled or self.is_fsdp_enabled
        ...
        tr_loss_sgd = torch.tensor(0.0).to(args.device)
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        self._total_loss_sgd_scalar = 0.0
        ...
        if args.hybrid_optimizer:

            # What parameters to optimize
            if args.fo_optimizer == 'sgd':
                self.optimizer = SGD(model.parameters(), lr=args.learning_rate)
            elif args.fo_optimizer == 'adam':
                self.optimizer = Adam(model.parameters(), lr=args.learning_rate, eps=1e-4)
            if self.args.layer_wise_hybrid:
                self.named_parameters_to_optim = []
                self.sgd_named_parameters_to_optim = []
                for name, param in model.named_parameters():
                    is_sgd_params = False
                    for name1 in self.args.sgd_optim_layers:
                        if self.args.lora:
                            if name1 in name and 'lora' in name:
                                self.sgd_named_parameters_to_optim.append((name, param))
                                logger.info(f'FO parameters: {name}')
                                is_sgd_params = True
                                break
                        else:
                            if name1 in name:
                                ...
                                is_sgd_params = True
                                break
                    if not is_sgd_params:
                        self.named_parameters_to_optim.append((name, param))
                        logger.info(f'ZO parameters: {name}')
                        param.requires_grad = False

            elif self.args.weight_wise_hybrid:
                self.named_parameters_to_optim = []
                for name, param in model.named_parameters():
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if args.trainer == "zo":
                    if args.hybrid_optimizer:
                        tr_loss_step, tr_loss_step1, tr_loss_step_sgd = self.hybrid_opt_step(model, inputs)
                    else:
                ...
                if args.hybrid_optimizer:
                    if (
                            ...
                            and not is_torch_xla_available() and tr_loss_step is not None
                            and (torch.isnan(tr_loss_step) or torch.isinf(tr_loss_step))
                    ):
                        # if loss is nan or inf simply add the average of previous logged losses
                        tr_loss += tr_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                    elif args.logging_nan_inf_filter and not is_torch_xla_available() \
                            and tr_loss_step_sgd is not None and (torch.isnan(tr_loss_step_sgd) or torch.isinf(tr_loss_step_sgd)):
                        tr_loss_sgd += tr_loss_sgd / (1 + self.state.global_step - self._globalstep_last_logged)
                    else:
                        if tr_loss_step is not None:
                            tr_loss += tr_loss_step
                        if tr_loss_step_sgd is not None:
                            tr_loss_sgd += tr_loss_step_sgd
                else:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if not args.hybrid_optimizer:
                        ...
                        if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                            ...
                            logs = {}
                            if tr_loss_step is not None:
                                logs["zo_loss1"] = tr_loss_step.item()
                                logs["zo_loss2"] = tr_loss_step1.item()

                            if tr_loss_step_sgd is not None:
                                logs["sgd_loss"] = tr_loss_step_sgd.item()
                                logs["sgd_learning_rate"] = self._get_learning_rate()
                            if tr_loss_step is not None:
                                logs["zo_learning_rate"] = args.zo_learning_rate
        ...
        # if args.hybrid_optimizer:
        #     self._total_loss_sgd_scalar += tr_loss_sgd.item()
        #     train_loss_sgd = self._total_loss_sgd_scalar / self.state.global_step
        ...
        # if args.hybrid_optimizer:
        #     metrics["train_loss_sgd"] = train_loss_sgd
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            mask = None
            if self.args.weight_wise_hybrid:
                if name in self.args.mask_dict:
                    mask = self.args.mask_dict[name]
            if mask is not None:
                index = torch.nonzero(mask.view(-1))[0]
                z = torch.normal(mean=0, std=1, size=index.shape[0], device=param.data.device, dtype=param.data.dtype)
                param.data.view(-1)[index] += scaling_factor * z * self.args.zo_eps
            else:
    ...
    ############## Hybrid optimizer ##############
    def hybrid_opt_step(self, model, inputs):
        """
        Estimate and compute gradient by MeZO and SGD. Return the loss from f(theta + z)
        """
        args = self.args
        if args.alternate_training:
            if self.state.global_step % 2 == 0:
                ...
                # Reset model back to its parameters at start of step
                self.zo_perturb_parameters(scaling_factor=1)
                return loss1, loss2, None
            else:
                sgd_loss = self.training_step(model, inputs)
                return None, None, sgd_loss
        else:
            ########## MeZO #############
            # Sample the random seed for sampling z
            self.zo_random_seed = np.random.randint(1000000000)
            ...
            ########## SGD #############
            if self.state.global_step % args.sgd_optim_step == 0:
                sgd_loss = self.training_step(model, inputs)
                return loss1, loss2, sgd_loss
            else:
                return loss1, loss2, None

    def hybrid_opt_update(self, model):
        """brid
        Update the parameters with the estimated gradients.
        """
        args = self.args
        if args.alternate_training:
            if self.state.global_step % 2 == 0:
                ...
                for name, param in self.named_parameters_to_optim:
                    ...
                    if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                        param.data = param.data - args.zo_learning_rate * (
                                    self.projected_grad * z + args.weight_decay * param.data)
                    else:
                        param.data = param.data - args.zo_learning_rate * (self.projected_grad * z)
            else:
                ################SGD################
                # Gradient clipping
                if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed:
            ...
            for name, param in self.named_parameters_to_optim:
                ...
                if mask is not None:
                    index = torch.nonzero(mask.view(-1))[0]
                    z = torch.normal(mean=0, std=1, size=index.shape[0], device=param.data.device,
                                     dtype=param.data.dtype)
                else:
                    z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device,
                                     dtype=param.data.dtype)
                ...
                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                    if mask is not None:
                        index = torch.nonzero(mask.view(-1))[0]
                        param.data.view(-1)[index] -= self.args.zo_learning_rate * (
                                self.projected_grad * z + self.args.weight_decay * param.data.view(-1)[index])
                    else:
                        param.data = param.data - args.zo_learning_rate * (self.projected_grad * z + args.weight_decay * param.data)
                else:
                    if mask is not None:
                        index = torch.nonzero(mask.view(-1))[0]
                        param.data.view(-1)[index] -= self.args.zo_learning_rate * (self.projected_grad * z)
                    else:
                        param.data = param.data - args.zo_learning_rate  * (self.projected_grad * z)
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        if is_torch_xla_available():
            ...
            "zero_dp_2" in self.args.sharded_ddp
            or "zero_dp_3" in self.args.sharded_ddp
            or self.fsdp is not None
        ):
