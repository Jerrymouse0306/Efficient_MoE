...
import time
import tasks
from transformers import AutoConfig, AutoTokenizer, AutoModelForCausalLM, AutoModelForSeq2SeqLM, HfArgumentParser, TrainingArguments, DataCollatorWithPadding, DataCollatorForTokenClassification # Trainer, 
...
import random
from masking.random_masking import RandomMasking
# add
import subprocess
import os
...
class OurArguments(TrainingArguments):
    ...
    only_predict: bool = False # whether only predict

    # MeZO
    zo_eps: float = 1e-3 # eps in MeZO
    hybrid_optimizer: bool = False # whether to use hybrid optimizer (e.g. zo & sgd) for fine-tuning.
    layer_wise_hybrid: bool = False # whether to use layer wise zo and fo hybrid optimization
    weight_wise_hybrid: bool = False # whether to use weight wise zo and fo hybrid optimization
    masking_prob: float = 0.99 # ratio of parameters updated by zo optimizer
    zo_learning_rate: float = 1e-6 # learning rate of MeZO
    sgd_optim_layers: List[str] = None # Parameters from which layers using sgd optimization
    sgd_optim_step: int = 200
    fo_optimizer: str = "sgd"
    alternate_training: bool = False # whether use ZO optimization and FO optimization alternately.
    if_sgd_dropout:  bool = True # whether to use dropout in FO optimization.
...
class Framework:
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            ...
            if not self.args.if_sgd_dropout:
                config.dropout = 0
            ...
            if self.args.head_tuning:
            ...
            elif self.args.only_predict:
                # No auto device (use for FSDP)
                model = AutoModelForCausalLM.from_config(
                    config=config,
                )
                rm = RandomMasking(model, self.args.masking_prob, self.args.sgd_optim_layers, fp16=True)
                self.args.mask_dict = rm.mask_dict
                if self.args.model_name:
                    full_state_dict = {}

                    # 列出文件夹中所有的权重文件
                    weight_files = [f for f in os.listdir(self.args.model_name) if
                                    f.startswith("pytorch_model") and f.endswith(".bin")]

                    # 循环遍历并加载每个权重文件
                    for weight_file in weight_files:
                        weight_path = os.path.join(self.args.model_name, weight_file)
                        # 加载当前分片的状态字典
                        state_dict_fragment = torch.load(weight_path)
                        # 更新到完整的状态字典中
                        full_state_dict.update(state_dict_fragment)
                ...
                # if self.args.model_name == "google/switch-base-8":
                ...
                #     # added
                #     offload_folder="autodl-tmp/offload",
        ...
        # Load tokenizer
        # if self.args.model_name == "google/switch-base-8":
        #     tokenizer = T5Tokenizer.from_pretrained(self.args.model_name, use_fast=False)
        # else:
        ...
        if self.args.hybrid_optimizer and self.args.weight_wise_hybrid and not self.args.only_predict:
            rm = RandomMasking(model, self.args.masking_prob, self.args.sgd_optim_layers, fp16=True)
            self.args.mask_dict = rm.mask_dict

        else:
            self.args.mask_dict = None
    ...
    def forward(self, input_ids, option_len=None, generation=False):
        ...
        if generation:
            ...
            if self.args.task_name == 'XSum':
                min_length = input_ids.size(1) + 20
                outputs = self.model.generate(
                    input_ids, do_sample=args.sampling, temperature=args.temperature, min_length=min_length,
